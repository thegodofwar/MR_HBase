...
public class SleepJob extends Configured implements Tool,  
  ...
  public static final Log LOG =
	    LogFactory.getLog(SleepJob.class);
  ...
  public void map(IntWritable key, IntWritable value,
    ...
    ++count;
    LOG.info(Thread.currentThread().getName()+" map count="+count);
    // output reduceSleepCount * numReduce number of random values, so that
    // each reducer will get reduceSleepCount number of keys.
    int k = key.get();
    LOG.info("map key="+key.get()+" value="+value.get());
  ...
  public void reduce(IntWritable key, Iterator<NullWritable> values,
    ...
    count++;
    LOG.info(Thread.currentThread().getName()+" reduce count="+count);
    while (values.hasNext()) {
    	LOG.info("reduce key="+key.get()+" value="+values.next().get());
    }
  }
